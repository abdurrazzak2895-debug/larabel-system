...
<div class="max-w-4xl">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-0.5">All platform notifications for users.</p>
        </div>
            ...
            @forelse ($notifications as $notification)
                <div class="px-6 py-4 flex items-start gap-4 {{ $notification->read_at ? 'opacity-60' : 'bg-indigo-50/40' }}">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-sm text-slate-500 mt-0.5">{{ $notification->body }}</p>
                        <p class="text-xs text-slate-400 mt-1">To: <span class="font-medium text-slate-600">{{ $notification->user?->name ?? 'N/A' }}</span></p>
                        @if (! $notification->read_at)
                            <form action="{{ route('admin.notifications.mark-read', $notification) }}" method="POST" class="mt-2">
                                @csrf
                                <button type="submit" class="inline-flex items-center gap-1.5 text-xs font-medium text-brand-600 hover:text-brand-700 transition">
