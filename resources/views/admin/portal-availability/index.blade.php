...
(() => {
    ...
    async function loadCenters(city, date) {
        ...
        try {
            ...
            const attempted = Array.isArray(result.accounts_attempted) ? result.accounts_attempted.length : 1;
            const fallbackUsed = result.fallback_used === true;
            const sourceLabel = fallbackUsed ? ` · checked ${attempted} account${attempted === 1 ? '' : 's'}` : '';
            centersMeta.textContent = `${city} · ${date} · ${rows.length} center slot${rows.length === 1 ? '' : 's'}${sourceLabel} · fetched ${result.fetched_at || 'now'}`;
            if (rows.length) {
                centersWrap.innerHTML = rows.map((center, index) => `<button type="button" class="center-card rounded-xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:border-brand-400 hover:bg-brand-50" data-index="${index}"><div class="flex items-start justify-between gap-3"><strong class="text-sm text-slate-900">${esc(center.test_center_name)}</strong><span class="rounded-full ${Number(center.available_seats) <= 3 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'} px-2 py-1 text-[11px] font-bold">${esc(center.available_seats)} seats</span></div><div class="mt-3 grid grid-cols-2 gap-3 text-xs text-slate-500"><span>Center ID<br><b class="text-slate-800">${esc(center.test_center_id)}</b></span><span>Test time<br><b class="text-slate-800">${esc(center.test_time || 'Not provided')}</b></span></div></button>`).join('');
                say(`${rows.length} live center slot${rows.length === 1 ? '' : 's'} loaded for ${city} on ${date}${fallbackUsed ? ` after checking ${attempted} ready accounts` : ''}.`, 'success');
            } else {
                centersWrap.innerHTML = `<div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-5 text-sm text-amber-800 md:col-span-2 xl:col-span-3"><strong>No live centers returned after bounded retries.</strong><span class="mt-1 block">Checked ${attempted} ready account${attempted === 1 ? '' : 's'}. The upstream portal may be temporarily unavailable, or this slot changed after the date search. Use “Refresh selected lookup” and try again; no stale slot is being shown.</span></div>`;
                say(`No live centers returned for ${city} on ${date} after checking ${attempted} ready account${attempted === 1 ? '' : 's'}. The upstream portal may be temporarily unavailable; try the selected lookup again.`, 'error');
            }
            ...
            if (error.name === 'AbortError') return;
            centersWrap.innerHTML = `<div class="rounded-xl border border-red-200 bg-red-50 px-4 py-5 text-sm text-red-800 md:col-span-2 xl:col-span-3"><strong>Live center lookup failed.</strong><span class="mt-1 block">${esc(error.message)} Try “Refresh selected lookup” after a short wait. No stale availability is being shown.</span></div>`;
            centersMeta.textContent = `${city} · ${date} · live lookup failed`;
            say(`Live center lookup failed for ${city} on ${date}. ${error.message} Try again after a short wait.`, 'error');
        }
    }
