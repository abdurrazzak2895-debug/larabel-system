    ...
    async function loadCenters(city, date) {
        ...
        try {
            ...
            if (rows.length) {
                ...
                say(`${rows.length} live center slot${rows.length === 1 ? '' : 's'} loaded for ${city} on ${date}.`, 'success');
            } else {
                centersWrap.innerHTML = '<div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-5 text-sm text-amber-800 md:col-span-2 xl:col-span-3"><strong>No live centers returned after bounded retries.</strong><span class="mt-1 block">The upstream portal may be temporarily unavailable, or this slot changed after the date search. Use “Refresh selected lookup” and try again; no stale slot is being shown.</span></div>';
                say(`No live centers returned for ${city} on ${date} after bounded retries. The upstream portal may be temporarily unavailable; try the selected lookup again.`, 'error');
            }
