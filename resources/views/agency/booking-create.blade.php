        ...
        function renderSessionShiftSummary(sessions) {
            ...
            const html = Object.keys(grouped).sort().map(function (date) {
                const rows = grouped[date].map(function (session, index) {
                    const centerId = sessionCenterId(session);
                    // A centerless session (this SVP deployment omits
                    // test_center id/name from the list response) is trusted
                    // as belonging to the requested center, matching
                    // resolveCenterSession's/verifyForHold's scoped-fallback
                    // logic on the backend. Only an EXPLICIT different
                    // center id is a real mismatch.
